...
#-----------------------------------------------------------------------------
vendor_saved_apps := Bluetooth CaptivePortalLogin KeyChain HTMLViewer UserDictionaryProvider BackupRestoreConfirmation \
                     FusedLocation PrintSpooler SharedStorageBackup  ExternalStorageProvider InputDevices \
                     ProxyHandler Shell DefaultContainerService CMSettingsProvider  WAPPushManager YGPS AudioFX Stk
...
#-----------------------------------------------------------------------------
board_saved_files := lib/libwebviewchromium.so lib64/libwebviewchromium.so bin/bootanimation media/bootanimation.zip bin/shutdownanimation
...
override_property += \
    ro.flyme.romer=bylgxws \
    ro.product.model_romer=p620m_bylgxws
